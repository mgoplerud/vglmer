...
vglmer <- function(formula, data, family, control = vglmer_control()) {
  ...
  if (length(parse_formula$smooth.spec) > 0){
    ...
    extra_Bj <- lapply(setdiff(names(names_of_RE), names(B_j)),
        FUN=function(i){Diagonal(n = d_j[i])})
  ...
  if (family == "linear") {
    ...
    vi_sigmasq_b <- (var(y) * (length(y)-1))/2 + vi_sigmasq_prior_b
    
    s <- y
    vi_pg_b <- rep(1, length(y))
    vi_pg_c <- NULL
    vi_r_mu <- 0
    vi_r_sigma <- 0
    vi_r_mean <- 0
  ...
  if ( control$init %in% c("EM", "EM_FE") ){
    if (family == "linear"){
      if (control$init == 'EM_FE'){
        EM_init <- solve(Cholesky(crossprod(X)), t(X) %*% y)
  ...
  if ( 
    ...
    Mmap <- do.call('cbind', mapply(outer_alpha_RE_positions[!spline_REs], parsed_RE_groups$factor, SIMPLIFY = FALSE, FUN=function(i,j){
      flat_i <- as.integer(sapply(i, min))
      flat_i[j]
    }))
    rownames(Mmap) <- NULL
  ...
  if (factorization_method %in% c("partially_factorized", "pf_diag")){
    ...
    size_Mj <- sum(lengths(M_j))
    size_Cj <- sum(lengths(C_j))
    ...
    # Extract the relevant matrices; design_C is made dense
    # by the assumption that is low rank
    design_C <- as.matrix(joint_XZ[, unlist(C_j), drop = F])
    design_M <- joint_XZ[, unlist(M_j), drop = F]
    gc()
    
    reverse_collapsing <- match(1:ncol(joint_XZ), c(unlist(C_j), unlist(M_j)))
    ...
    vi_FS_MM <- c(
      list('Fixed Effect' = FS(vi_M_list[["Fixed Effect"]], vi_M_list[["Fixed Effect"]])),
      vi_FS_MM
    )
    vi_FS_MM <- lapply(vi_FS_MM, as.matrix)
    ...
    if (linpred_method == 'cyclical'){
      ...
      vi_M_B <- lapply(vi_P, 
       FUN=function(i){
         if (ncol(i) > 0 & nrow(i) > 0){
           drop0(sparseMatrix(i = 1, j = 1, x = 0, dims = dim(i)))
         }else if (nrow(i) == 0){
           drop0(matrix(nrow = 0, ncol = ncol(i)))
         }else{
      ...
      # if (any_collapsed_C){
      #   vi_M_B$`Fixed Effect` <- rep(0, ncol(vi_M_list$`Fixed Effect`)^2)
      # }
      vi_B_raw <- vi_M_B
      ...
      if (nrow(vi_M_var_flat[["Fixed Effect"]]) != 0){
        vi_M_var_flat$`Fixed Effect` <- matrix(0, nrow = 1, ncol = ncol(vi_M_list$`Fixed Effect`)^2)
      }
  ...
  for (it in 1:iterations) {
      ...
      if (factorization_method %in% c("weak")) {
        ...
        if (linpred_method == 'cyclical'){
          ...
          joint_quad <- cpp_var_lp_joint(design_C = design_C,
             ...
             sparse_input = class(vi_M_var[[1]])[1] %in% c('dgCMatrix', 'generalMatrix')
          )
        }
    ...
    if (factorization_method == "weak") {
      ...
      if (linpred_method == 'cyclical'){
        if (any_collapsed_C){
          ...
          chol.update.C <- Cholesky(crossprod(sqrt(diag_vi_pg_mean) %*% design_C) + Tinv_C)
          C_hat <- as.vector(solve(chol.update.C, t(design_C) %*% s))
          vi_C_var_decomp <- solve(expand1(chol.update.C, 'L')) %*% expand1(chol.update.C, 'P1')
          ...
          log_det_C_var <- -2 * sum(log(diag(expand1(chol.update.C, 'L'))))
          pg_lp_C_hat <- diag_vi_pg_mean %*% design_C %*% C_hat
        }else{
          Tinv_C <- matrix(0, nrow = 0, ncol = 0)
          log_det_C_var <- 0
          pg_lp_C_hat <- rep(0, nrow(X))
          C_hat <- numeric(0)
        }
        ...
        for (j in seq_len(length(M_j))) {
          ...
          old_j <- solve(drop0(t(data_M_j) %*% diag_vi_pg_mean %*% data_M_j +
                                 bdiag(Tinv)[index_Mj, index_Mj]))
          
          P_j <- vi_P[[j]]
          
          # data_projM_j <- data_M_j - design_C %*% vi_P[[j]]
          # var_Mj <- solve(t(data_projM_j) %*% diag_vi_pg_mean %*% data_projM_j +
          #                   bdiag(Tinv)[index_Mj, index_Mj] + t(vi_P[[j]]) %*% Tinv_C %*% vi_P[[j]])
          
          if (j == 1){
            if (block_collapse){stop('set up indexing for that case')}
            ZtZinv_j <- solve(t(data_M_j) %*% diag_vi_pg_mean %*% data_M_j)
            ZtZinv_j <- list(
              inverse = ZtZinv_j,
              det = as.numeric(determinant(ZtZinv_j)$modulus)
            )
          }else{
            ZtZinv_j <- invert_rowwise(
              X = as.matrix(t(lookup_marginal[[j]]) %*% diag_vi_pg_mean %*% vi_FS_MM[[j]]),
              vec_prior = matrix(as.vector(Tinv[index_Mj[1:aug_dj[j]], index_Mj[1:aug_dj[j]]])),
              dim_X = aug_dj[j]
            )
          }  
          ...
          if (any_collapsed_C){
            ...
            meat_C <- inner_meat_C +
              - t(bread_j) %*% old_j %*% bread_j + Tinv_C
            if (!isSymmetric(meat_C)){
              meat_C <- symmpart(meat_C)
            }
            ...
            if (factorization_method == 'pf_diag'){
              ...
              running_log_det_M_var[j] <- -2 * as.numeric(determinant(chol_Cmeat)$modulus) + 
                -log_det_C_var + ZtZinv_j$det
              
            }else if (d_jm == 1){
              
              var_flat_j <- diag(old_j) + rowSums( scaled_ZX_j * t(B_term_2) )
              var_flat_j <- matrix(var_flat_j)
              running_log_det_M_var[j] <- -2 * as.numeric(determinant(chol_Cmeat)$modulus) + 
                -log_det_C_var + 2 * sum(ZtZinv_j$det)
              
            }else{
              var_flat_j <- ZtZinv_j$inverse +
                block_diag_product(A = as.matrix(scaled_ZX_j), 
                                   B = as.matrix(t(B_term_2)),
                                   blocks = ncol(data_M_j)/d_jm, block_size = d_jm)
              running_log_det_M_var[j] <- -2 * as.numeric(determinant(chol_Cmeat)$modulus) + 
                -log_det_C_var + sum(ZtZinv_j$det) * 2
            }
            ...
            if (j == 1){
              if (block_collapse){stop('...')}
              var_flat_j <- matrix(as.vector(ZtZinv_j$inverse), nrow = 1)
              running_log_det_M_var[j] <- sum(ZtZinv_j$det)
            }else{
              var_flat_j <- ZtZinv_j$inverse
              running_log_det_M_var[j] <- 2 * sum(ZtZinv_j$det)
            }
          ...
          if (j == 1){
            ...
            # vvv <- max(abs(range(var_Mj - matrix(as.vector(var_flat_j), ncol = ncol(var_Mj)))))
            # if (vvv > 1e-8){browser()}
          }else{
            # if (max(abs(diag(var_Mj) - var_flat_j[,1])) > 1e-8){
            #   browser()
            # }
          }
          if (any_collapsed_C){
      ...
      if (linpred_method == "joint") {
        ...
        for (j in 1:number_of_RE) {
          ...
          vi_alpha_decomp <- fast_insert(
            A = solve(chol.j$origL) %*% t(Pmatrix), 
            B = vi_alpha_decomp, 
            index = index_j)
    ...
    if (family == 'linear'){
      ...
      if (factorization_method %in% c("partially_factorized", "pf_diag")){
        ...
        if (linpred_method == 'joint'){
          ...
          vi_M_B <- lapply(vi_M_B, FUN=function(i){i * adjust_var^2})
        }
        
        vi_C_var <- vi_C_var * adjust_var^2
        vi_C_uncond <- vi_C_uncond * adjust_var^2
        
        log_det_M_var <- log_det_M_var + size_Mj * ln_sigmasq
        log_det_C_var <- log_det_C_var + size_Cj * ln_sigmasq
      }else{
    ...
    if (debug_ELBO & it != 1) {
      ...
      if (factorization_method %in% c("partially_factorized", "pf_diag")){
        ...
        ssq_out <- prepare_total_variance(vi_C_mean = vi_C_mean, 
          vi_M_mean = vi_M_mean,
          vi_M_var_flat = vi_M_var_flat, aug_dj = aug_dj,
          any_collapsed_M = any_collapsed_M, 
          any_collapsed_C = any_collapsed_C,
          vi_C_uncond = vi_C_uncond, 
          position_block_j = position_block_j, c_id_Mj = c_id_Mj,
    ...
    if (family == "negbin") {
      ...
      if (factorization_method == 'weak'){
        ...
        if (any(joint_quad < 0)){browser()}
        
        vi_lp <- design_C %*% vi_C_mean 
        if (ncol(design_M) > 0){
          vi_lp <- vi_lp + design_M %*% do.call('c', vi_M_mean) 
        }
        vi_lp <- (s - vi_lp)^2 + joint_quad
    ...
    if (debug_ELBO) {
      debug_ELBO.3 <- calculate_ELBO(family = family,
        ...
        vi_joint_decomp = vi_joint_decomp,
        vi_P = vi_P, vi_C_uncond = vi_C_uncond, 
        design_C = design_C, design_M = design_M,
        vi_FS_MM = vi_FS_MM, vi_FS_MC = vi_FS_MC, lookup_marginal = lookup_marginal,
        vi_M_mean = vi_M_mean, vi_C_mean = vi_C_mean, 
        vi_M_B = vi_M_B, vi_M_var_flat = vi_M_var_flat,
    ...
    if (parameter_expansion == "none" | !any_Mprime) {
      ...
      if (factorization_method %in% c("partially_factorized", "pf_diag")){
        ...
        stop('Set up parameter-expansion for partially factorized')
      }else{
        
        # Do a simple mean adjusted expansion; allow for
        # random effects without a corresponding fixed effect
    ...
    if (parameter_expansion %in% c("translation", "diagonal") & skip_translate == FALSE & any_Mprime) {
      ...
      if (factorization_method == 'weak'){
        stop('No translation parameter expansion for "weak" yet...')
      }
      ...
      if (factorization_method != 'weak'){
        ...
        prop_variance_by_alpha_jg <- calculate_expected_outer_alpha(
          factorization_method = factorization_method,
          alpha_decomp_var = prop_vi_alpha_decomp, 
          # alpha_var = prop_vi_alpha_var,
          alpha_mu = as.vector(prop_vi_alpha_mean),
          re_position_list = outer_alpha_RE_positions,
          L_beta = as.matrix(prop_vi_beta_decomp), tP = t(prop_vi_collapsed_P), 
          do_adjustment = adjust_ceoa)
      ...
      if (debug_px){
        prop.ELBO <- calculate_ELBO(family = family,
          ...
          # vi_alpha_var = prop_vi_alpha_var, 
          # vi_beta_var = prop_vi_beta_var, 
          cyclical_pos = cyclical_pos,
          ...
          # # set up for partially factorized
          # vi_P = XX_vi_P, vi_C_uncond = XX_vi_C_uncond, 
          # vi_FS_MM = vi_FS_MM, vi_FS_MC = vi_FS_MC, lookup_marginal = lookup_marginal,
          # vi_M_mean = XX_vi_M_mean, vi_C_mean = XX_vi_C_mean, 
          # design_M = design_M, 
          # vi_M_B = XX_vi_M_B, vi_M_var_flat = XX_vi_M_var_flat,
          # design_C = design_C, log_det_M_var = XX_log_det_M_var, 
          # log_det_C_var = XX_log_det_C_var,
        ...
        for (attempt_SQUAREM in 1:10){
          ...
          if ('vi_pg_c' %in% squarem_par){
            ...
            if (factorization_method %in% c("weak")) {
              ...
              joint_quad <- cpp_zVz(Z = joint.XZ, V = as(prop_ELBOargs$vi_joint_decomp, "generalMatrix")) 
  ...
  if (debug_param) {
    ...
    output$parameter_trajectory <- list(
      beta = store_beta, alpha = store_alpha,
      sigma = store_sigma, hw = store_hw)
  }
  ...
  if (factorization_method == "weak") {
    ...
    if (linpred_method == 'joint'){
      ...
      if (any_collapsed_M){
        ...
        vi_C_uncond <- vi_C_var + add_adjust
      }else{
        vi_C_uncond <- vi_C_var
      }
    ...
    # Save the index to reverse collapsing; needed 
    # for drawing samples from posterior
    output$marginal$reverse_collapsing <- reverse_collapsing
    ...
    # Check that the "reversing" vector works as expected
    reverse_mean <- c(as.vector(vi_C_mean), as.vector(unlist(vi_M_mean)))[reverse_collapsing]
    stopifnot(all.equal(as.numeric(unlist(fmt_mean)), reverse_mean))    
    
    if (linpred_method == 'joint'){
      var_M <- lapply(vi_M_var, diag)
      stop('delete joint...')
    }else{
      var_M <- lapply(vi_M_var_flat, 
        FUN=function(i){
          if (ncol(i) != 0){
            d <- sqrt(ncol(i))
            p <- cumsum(c(1, rep(d+1, d-1)))  
            return(as.vector(t(i[,p])))
          }else{return(NULL)}
        })
    }
    if (block_collapse){
    ...
    fmt_var <- mapply(var_C, var_M, C_j, M_j, SIMPLIFY = FALSE,
       FUN=function(ci, mi,index_c, index_m){
         c(ci, mi)[order(c(index_c, index_m))]
       })
    ...
    if (ncol(X) > 0){
      if (block_collapse){stop('....')}
      B_FE <- matrix(vi_M_B$`Fixed Effect`, 
        ncol = ncol(vi_M_list$`Fixed Effect`), byrow = TRUE)
      
      if (any_collapsed_C){
        uncond_FE_C <- vi_C_uncond[which(position_block_j == 0), which(position_block_j == 0), drop = F]
        if (is.null(uncond_FE_C)){uncond_FE_C <- matrix(0, nrow=0, ncol=0)}
      }else{
        uncond_FE_C <- matrix(0, nrow=0, ncol=0)
      }
      if (any_collapsed_M){
        uncond_FE_M <- matrix(vi_M_var_flat$`Fixed Effect`, ncol = ncol(vi_M_list$`Fixed Effect`))
        if (is.null(uncond_FE_M)){uncond_FE_M <- matrix(0, nrow =0, ncol = 0)}
      }else{
        uncond_FE_M <- matrix(0, nrow=0,ncol=0)
      }

      if (ncol(uncond_FE_C) & ncol(uncond_FE_M)){
        uncond_FE_cov <- B_FE[which(position_block_j == 0),,drop=F]
        uncond_FE <- rbind(
          cbind(uncond_FE_C, uncond_FE_cov),
          cbind(t(uncond_FE_cov), uncond_FE_M)
        )
      }else{
        uncond_FE <- as.matrix(bdiag(uncond_FE_C, uncond_FE_M))
      }
      
      order_FE <- order(c(C_j$`Fixed Effect`, M_j$`Fixed Effect`))
      uncond_FE <- uncond_FE[order_FE, order_FE, drop = F]
      rownames(uncond_FE) <- colnames(uncond_FE) <- colnames(X)
      output$beta$mean <- matrix(fmt_mean$`Fixed Effect`)
      output$beta$var <- uncond_FE
  ...
  if (control$return_data) {
    ...
    if (factorization_method %in% c('partially_factorized', 'pf_diag')){
      ...
      output$data$vi_P <- vi_P
      output$data$aug_dj <- aug_dj
    }
  }
  ...
  if (factorization_method %in% c('partially_factorized', 'pf_diag')){
    ...
    if (any_collapsed_M){
      confirm_lp <- confirm_lp + design_M %*% do.call('c', vi_M_mean)
    }
    confirm_lp <- as.vector(confirm_lp)
    stopifnot(all.equal(final_lp, confirm_lp))
  }else{
    final_lp <- as.vector(X %*% vi_beta_mean + Z %*% vi_alpha_mean - vi_r_mu)
  }
...
#'   Described in detail in Goplerud (2022). \code{"strong"},
#'   \code{"intermediate"}, and \code{"weak"} correspond to Schemes I, II, and
#'   III respectively in that paper. \code{"partially_factorized"} corresponds
#'   to the partially collapsed scheme discussed in Goplerud, Papaspiliopoulos,
#'   and Zanella (2023). Some features (e.g., splines) are currently disabled
#'   for \code{"partially_factorized"} models.
#' @param collapse_set If \code{factorization_method = "partially_factorized"},
#'   this defines the set of random or fixed effects that are put in the
#'   collapsed set; see Goplerud, Papaspiliopoulos, and Zanella (2023) for more
#'   details. The default \code{"FE"} puts only the fixed effects in the
#'   collapsed set. This can be specified using the names of random effects.
...
#'   \code{limit}) is used.
#' @param drop.unused.levels Default (\code{TRUE}) drops unused factor levels
#'   for the random effects. Set to \code{FALSE} to estimate parameters all
#'   factor levels even if they have no observations.
...
#' 17(2): 623-650.
#'
#' Goplerud, Max. 2023. "Re-Evaluating Machine Learning for MRP Given the
#' Comparable Performance of (Deep) Hierarchical Models." \emph{American
#' Political Science Review}. Advance Access. DOI:
#' \url{https://doi.org/10.1017/S0003055423000035}
#'
#' Goplerud, Max, Omiros Papaspiliopoulos, and Giacomo Zanella. 2023. "Accurate
#' and Scalable Variational Inference for Generalized Linear Mixed Models."
#' Working Paper.
#' 
#' Huang, Alan, and Matthew P. Wand. 2013. "Simple Marginally Noninformative
#' Prior Distributions for Covariance Matrices." \emph{Bayesian Analysis}.
...
vglmer_control <- function(iterations = 1000, 
  ...
  if (factorization_method %in% c('partially_factorized', 'pf_diag')){
    ...
    if (do_SQUAREM){
      message('Setting do_SQUAREM=FALSE for "partially_factorized"')
      do_SQUAREM <- FALSE
    }
    if (parameter_expansion != 'none'){
      message('Setting parameter_expansion="none" for "partially_factorized"')
