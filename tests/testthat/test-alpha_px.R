...
test_that('Calculate E[alpha alpha^T]', {
  ...
  legacy_method <- mapply(legacy_method[[1]], legacy_method[[2]], SIMPLIFY = FALSE, FUN=function(a,b){a+b})
  ...
  comp <- mapply(legacy_method, cpp_method, FUN=function(i,j){all.equal(as.vector(i), as.vector(j))})
